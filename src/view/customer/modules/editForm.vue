      ...
      <Row>
        <Col span="12">
          <Form-item label="上级公司" prop="parentId">
            <!-- <div v-if="moadl_status === 3">{{ detail.companyName }}</div> -->
            <Select v-model="formData.parentId" :disabled="moadl_status === 3" placeholder="请选择上级公司">
        ...
        <Col span="12">
          <Form-item  label="公司名称" prop="companyName">
            <Input v-model="formData.companyName" :disabled="moadl_status === 3" placeholder="请输入公司名称"></Input>
          </Form-item>
        </Col>
      </Row>
      <Row>
        <Col span="12">
          <Form-item label="公司类型" prop="companyType">
            <Select v-model="formData.companyType" :disabled="moadl_status === 3" placeholder="请选择公司类型">
          ...
          <Col span="12">
            <Form-item label="法人代表" prop="legalPerson">
              <Input v-model="formData.legalPerson" :disabled="moadl_status === 3" placeholder="请输入法人代表"></Input>
            </Form-item>
          </Col>
      </Row>
      <Row>
        <Col span="12">
          <Form-item label="公司地址" prop="area" class="mult_select_box">
            <Select v-model="formData.province" :disabled="moadl_status === 3" placeholder="-省-" @on-change="changeProvince" class="address_select">
              <Option v-for="(item, index) in provinceList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.city" :disabled="moadl_status === 3" placeholder="-市-" @on-change="changeCity" class="address_select">
              <Option v-for="(item, index) in cityList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.area" :disabled="moadl_status === 3" placeholder="-区/县-" class="address_select">
        ...
        <Col span="12">
          <Form-item label="详细地址" prop="companyAddress">
            <Input v-model="formData.companyAddress" :disabled="moadl_status === 3" placeholder="请输入详细地址"></Input>
          </Form-item>
        </Col>
      </Row>
      <Row>
        <Col span="12">
          <Form-item label="社保规则地区" prop="area1" class="mult_select_box">
            <Select v-model="formData.province1" :disabled="moadl_status === 3" placeholder="-省-" @on-change="changeSocialProvince" class="address_select">
              <Option v-for="(item, index) in provinceList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.city1" :disabled="moadl_status === 3" placeholder="-市-" @on-change="changeSocialCity" class="address_select">
              <Option v-for="(item, index) in socialCityList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.area1" :disabled="moadl_status === 3" placeholder="-区/县-" @on-change="changeSocialArea" class="address_select">
        ...
        <Col span="12">
          <Form-item label="社保规则" prop="socialNO">
            <Select v-model="formData.socialNO" :disabled="moadl_status === 3" placeholder="请选择社保规则">
      ...
      <Row>
        <Col span="12">
          <Form-item label="公积金地区" prop="area2" class="mult_select_box">
            <Select v-model="formData.province2" :disabled="moadl_status === 3" placeholder="-省-" @on-change="changePublicProvince" class="address_select">
              <Option v-for="(item, index) in provinceList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.city2" :disabled="moadl_status === 3" placeholder="-市-" @on-change="changePublicCity" class="address_select">
              <Option v-for="(item, index) in publicCityList" :value="item.value" :key="index">{{ item.label }}</Option>
            </Select>
            <Select v-model="formData.area2" :disabled="moadl_status === 3" placeholder="-区/县-" @on-change="changePublicArea" class="address_select">
        ...
        <Col span="12">
          <Form-item label="公积金规则" prop="publicFundNO">
            <Select v-model="formData.publicFundNO" :disabled="moadl_status === 3" placeholder="请选择社保规则">
      ...
      <Row>
        <Col span="12">
          <Form-item label="有效期限" prop="expiryDateType">
            <Select v-model="formData.expiryDateType" :disabled="moadl_status === 3" placeholder="请选择有效期限">
        ...
        <Col span="12">
          <Form-item label="有效期限时间" prop="expiryDate">
            <Date-picker :value="formData.expiryDate" :disabled="moadl_status === 3" type="date" :editable="false" format="yyyy-MM-dd" @on-change="choiceDate" placeholder="选择有效期限时间"></Date-picker>
      ...
      <Row>
        <Col span="12">
          <Form-item label="社会信用代码" prop="netpNo">
            <Input v-model="formData.netpNo" :disabled="moadl_status === 3" placeholder="请输入社会信用代码"></Input>
          </Form-item>
        </Col>
        <Col span="12">
          <Form-item label="所属行业" prop="industry">
            <Select v-model="formData.industry" :disabled="moadl_status === 3" placeholder="请选择所属行业">
      ...
      <Row>
        <Col span="12">
          <Form-item label="对公帐号" prop="billBankno">
            <Input v-model="formData.billBankno" :disabled="moadl_status === 3" placeholder="请输入对公帐号"></Input>
          </Form-item>
        </Col>
        <Col span="12">
          <Form-item label="开户行" prop="billBankaddr">
            <Input v-model="formData.billBankaddr" :disabled="moadl_status === 3" placeholder="请输入开户行"></Input>
          </Form-item>
        </Col>
      </Row>
      <Row>
        <Col span="12">
          <Form-item label="联系电话" prop="mobile">
            <Input v-model="formData.mobile" :disabled="moadl_status === 3" placeholder="请输入联系电话"></Input>
          </Form-item>
        </Col>
        <Col span="12">
          <Form-item label="销售人员" prop="saleId">
            <Select v-model="formData.saleId" :disabled="moadl_status === 3" placeholder="请选择销售人员">
